function range = findnode(node,range,varargin)
...
hold on
dotColor = 'r.';
if (nargin==1) || isempty(range) || (ischar(range) && strcmp(range,'all'))
    range = (1:size(node,1))'; 
    dotColor = 'k.';
end
if size(node,2) == 3
...
h = plot(node(range,1),node(range,2),dotColor, 'MarkerSize', 24);
if nargin>2
    startidx = 1;
