...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_applyprec( N,f,X,a,ja,ia)
    USE zagmg_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    CALL zagmg_prec_matv(1,S,X,S(N+1),S(2*N+1),.FALSE.)
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       CALL zagmg_prec_matv(1,fsc,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
       S(1+3*N:4*N)=S(1+3*N:4*N)*scald(1:N)
       flop=flop+dble(2*N)
       IF ( ITER > 1 ) THEN
          dum(1) = - ZDOTC(NBLAP, SD, IONE, S(1+3*N), IONE)
          BET0=dum(1)
          BET1=BET0/RHO
       ...
       !
       dum(1) =  ZDOTC(NBLAP, S(1+2*N), IONE, SD, IONE)
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       IF (itm > 0) THEN
          DO i=0,itm-1
             dum(1)=ZDOTC(NBLAP,Sc(1+i*N),IONE,Sc(1+itm*N),IONE)
             bet0=dum(1)
       ...
       dum(2)=ZDOTC(NBLAP, Sc(1+itm*N), IONE, f, IONE)
       IF (ITER == 1) THEN
          IF (init == 0) THEN
             BNORM2=dble(dum(3))
             RESID2=BNORM2
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE zagmg_prec_matv(l,B,X,AX,R,matv)
    USE zagmg_mem
    IMPLICIT NONE
    INTEGER :: l
...
100 CONTINUE
    ...
    dum(2)=ZDOTC(NBLAP, R(1,2), IONE, R, IONE )
    BNORM = dble(dum(3))
110 CONTINUE
    rho1=dum(1)
    alpha1=dum(2)
    ...
    !
    !
    dum(1) = ZDOTC(NBLAP,R(1,4),IONE,R(1,2),IONE)
    dum(2) = ZDOTC(NBLAP,R(1,4),IONE,R,IONE)
...
200 CONTINUE
    ...
    !
    dum(1) = DZNRM2(NBLAP,R(1,2),IONE)**2
    dum(2) = ZDOTC(NBLAP,R(1,4),IONE,R(1,2),IONE)
    dum(3) = DZNRM2(NBLAP,R(1,4),IONE)**2
    dum(4) = ZDOTC(NBLAP,R(1,2),IONE,R,IONE)
    dum(5) = ZDOTC(NBLAP,R(1,4),IONE,R,IONE)
     !
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL zagmg_MUMPSseq(n,fff,1,a,ja,ia)
       IF (wfo) THEN
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL zagmg_MUMPSseq(n,fff,1,dt(l)%a,dt(l)%ja,dt(l)%ia)
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmg_MUMPSseq(n,f,ijob,a,ja,ia)
    USE zagmg_mem
    IMPLICIT NONE
!!!!!!!
!
...
!
    ...
    IF (preprocessed) THEN
       CALL zagmg_relmem
       IF (.NOT.allzero)                        &
               CALL zagmg_MUMPSseq(nn(nlev),fdum,-2)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          ...
          call mexPrintf(lout//char(10))
       END IF
       CALL zagmg_applyprec(n,f,x,a,ja,ia)
