...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_applyprec( N,f,X,a,ja,ia)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER       :: N
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       !
       CALL dagmg_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
       IF ( ITER > 1 ) THEN
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       !
       CALL dagmg_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,.FALSE.)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE dagmg_prec_matv(l,B,X,AX,R,matv)
    USE dagmg_mem
    IMPLICIT NONE
    INTEGER :: l
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL dagmg_MUMPSseq(n,fff,1,a,ja,ia)
       IF (wfo) THEN
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL dagmg_MUMPSseq(n,fff,1,dt(l)%a,dt(l)%ja,dt(l)%ia)
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmg_MUMPSseq(n,f,ijob,a,ja,ia)
    USE dagmg_mem
    IMPLICIT NONE
!!!!!!!
!
...
!
    ...
    IF (preprocessed) THEN
       CALL dagmg_relmem
       IF (.NOT.allzero)                        &
               CALL dagmg_MUMPSseq(nn(nlev),fdum,-2)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0d0
       cputm=0.0d0
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          ...
          call mexPrintf(lout//char(10))
       END IF
       CALL dagmg_applyprec(n,f,x,a,ja,ia)
